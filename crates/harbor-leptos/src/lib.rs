...
mod tests {
    ...
    use super::{
        CookieDefaults, CookieName, Harbor, HeaderName, PublicBaseUrl, SameSite,
        UnixTimestampMicros, build_csrf_cookie, build_delete_csrf_cookie,
    ...
    fn cookie_headers_cover_variants_and_rejections() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(
            super::build_cookie_header(strict.session_cookie_name(), "", &strict, true, None,)
                .is_err()
        );
        assert!(
