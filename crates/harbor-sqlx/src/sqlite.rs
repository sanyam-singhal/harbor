...
use harbor_core::{
    AccountStore, AppendAuthEventInput, AuthEventRecord, AuthEventStore, CanonicalEmail,
    ChallengeId, ChallengeRecord, ChallengeStore, CreateChallengeInput, CreatePasswordUserInput,
    CreatePasswordUserOutput, CreateSessionInput, CreateUserEmailInput, CreateUserInput,
    CreateVerifiedEmailUserInput, CreateVerifiedEmailUserOutput, DeleteExpiredSessionsInput,
    ...
    RateLimitStore, RedirectPath, RevokeSessionInput, RevokeUserSessionsInput, SessionId,
    SessionRecord, SessionStore, StoreError, StoreErrorCode, TokenHash, UnixTimestampMicros,
    UpdateSessionLastSeenInput, UserEmailRecord, UserEmailStore, UserId, UserRecord, UserStore,
};
use sqlx::SqlitePool;
...
mod codec;
use codec::{
    auth_event_kind_to_db, challenge_delivery_to_db, challenge_from_row, challenge_purpose_to_db,
    email_from_row, get_i64, map_sqlx_error, password_from_row, session_from_row, user_from_row,
};
#[cfg(test)]
use codec::{challenge_delivery_from_db, challenge_purpose_from_db, map_domain_error};
...
impl AccountStore for SqliteAuthStore {
    fn create_password_user(
        ...
        async move {
            let mut transaction = pool
                .begin()
                .await
                .map_err(|error| map_sqlx_error(error, "create_password_user_begin"))?;
            sqlx::query(
                "INSERT INTO harbor_users \
                 (id, created_at_unix_micros, updated_at_unix_micros, disabled_at_unix_micros) \
                 VALUES (?1, ?2, ?3, NULL)",
            ...
            .await
            .map_err(|error| map_sqlx_error(error, "create_password_user_user"))?;
            ...
            .execute(&mut *transaction)
            .await
            .map_err(|error| map_sqlx_error(error, "create_password_user_email"))?;
            ...
            .execute(&mut *transaction)
            .await
            .map_err(|error| map_sqlx_error(error, "create_password_user_password"))?;

            transaction
                .commit()
                .await
                .map_err(|error| map_sqlx_error(error, "create_password_user_commit"))?;

            Ok(CreatePasswordUserOutput {
                user: UserRecord {
    ...
    fn create_verified_email_user(
        ...
        async move {
            let mut transaction = pool
                .begin()
                .await
                .map_err(|error| map_sqlx_error(error, "create_verified_email_user_begin"))?;
            sqlx::query(
                "INSERT INTO harbor_users \
                 (id, created_at_unix_micros, updated_at_unix_micros, disabled_at_unix_micros) \
                 VALUES (?1, ?2, ?3, NULL)",
            ...
            .execute(&mut *transaction)
            .await
            .map_err(|error| map_sqlx_error(error, "create_verified_email_user_user"))?;
            ...
            .execute(&mut *transaction)
            .await
            .map_err(|error| map_sqlx_error(error, "create_verified_email_user_email"))?;

            transaction
                .commit()
                .await
                .map_err(|error| map_sqlx_error(error, "create_verified_email_user_commit"))?;

            Ok(CreateVerifiedEmailUserOutput {
                user: UserRecord {
                    id: input.user_id.clone(),
                    created_at: input.now,
                    updated_at: input.now,
                ...
                email: UserEmailRecord {
                    ...
                    email_original: input.email_original,
                    email_canonical: input.email_canonical,
                    verified_at: Some(input.verified_at),
                    is_primary: true,
                    created_at: input.now,
                    updated_at: input.now,
                },
